...
// The fork gate turns on this one answer, so a token that cannot push has to
// come back false rather than as an error, and a response with no permissions
// object at all has to come back false rather than as the zero value read as a
// yes by accident.
...
// decides it is Writable, not this.
